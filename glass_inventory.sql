...
PRIMARY KEY (`id`)) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci AUTO_INCREMENT = 193 ROW_FORMAT = Dynamic COMMENT = "库位架信息表";
...
CREATE TABLE `glass_inventory`.`inventory_check_tasks` (
    ...
    PRIMARY KEY (`id`),
    INDEX `idx_tasks_base_created`(`base_id`, `created_at`)
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '盘点任务表';

-- glass_inventory.inventory_check_cache DDL (完整版本)
CREATE TABLE `glass_inventory`.`inventory_check_cache` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    ...
    KEY `idx_check_cache_task_status`(`task_id`, `check_quantity`, `system_quantity`)
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '盘点明细缓存表';

-- glass_inventory.inventory_check_results DDL
CREATE TABLE `glass_inventory`.`inventory_check_results` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `task_id` INT(11) NOT NULL COMMENT '盘点任务ID',
    ...
    KEY `idx_task_id`(`task_id`),
    KEY `idx_results_task_glass`(`task_id`, `glass_type_id`)
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '盘点结果汇总表';

-- glass_inventory.inventory_check_settings DDL
CREATE TABLE `glass_inventory`.`inventory_check_settings` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '盘点配置表';
...
CREATE  ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `glass_inventory`.`inventory_check_task_summary` AS 
SELECT 
    t.`id` AS `id`,
    ...
    t.`status` AS `status`,
    t.`total_packages` AS `total_packages`,
    t.`checked_packages` AS `checked_packages`,
    CASE 
    ...
    CASE 
        ...
        ELSE NULL
    END AS `duration`
FROM ((`glass_inventory`.`inventory_check_tasks` `t` 
LEFT JOIN `glass_inventory`.`bases` `b` ON((`t`.`base_id` = `b`.`id`))) 
LEFT JOIN `glass_inventory`.`users` `u` ON((`t`.`created_by` = `u`.`id`)));

-- glass_inventory.inventory_check_difference_details DDL
CREATE  ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `glass_inventory`.`inventory_check_difference_details` AS 
SELECT 
    c.`task_id` AS `task_id`,
    c.`package_code` AS `package_code`,
    g.`short_name` AS `glass_name`,
    c.`system_quantity` AS `system_quantity`,
    c.`check_quantity` AS `check_quantity`,
    c.`difference` AS `difference`,
    CASE 
        WHEN c.`difference` > 0 THEN CONCAT('+', c.`difference`)
        WHEN c.`difference` = 0 THEN '0'
        ELSE CAST(c.`difference` AS CHAR CHARSET utf8mb4)
    ...
    r.`code` AS `rack_code`,
    c.`check_method` AS `check_method`,
    c.`check_time` AS `check_time`,
    u.`real_name` AS `operator_name`
FROM ((((`glass_inventory`.`inventory_check_cache` `c` 
LEFT JOIN `glass_inventory`.`glass_packages` `p` ON((`c`.`package_id` = `p`.`id`))) 
LEFT JOIN `glass_inventory`.`glass_types` `g` ON((`p`.`glass_type_id` = `g`.`id`))) 
LEFT JOIN `glass_inventory`.`storage_racks` `r` ON((`c`.`rack_id` = `r`.`id`))) 
LEFT JOIN `glass_inventory`.`users` `u` ON((`c`.`operator_id` = `u`.`id`))) 
...
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `glass_inventory`.`AddForeignKeySafely`()
BEGIN
