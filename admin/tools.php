...
function processExcelImport($filePath, $currentUser)
{
    $results = [
        ...
        'packages' => ['success' => 0, 'error' => 0, 'errors' => []]
    ];
    ...
    try {
        ...
        if ($spreadsheet->getSheetCount() > 3) {
            ...
            $results['packages'] = importPackages($sheet);
        }
...
function importGlassTypes($sheet)
{
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        try {
            ...
            // 完整的字段验证（只验证必填字段）
            if (empty($customId)) {
                $result['errors'][] = "第{$row}行：原片ID不能为空";
                $result['error']++;
                continue;
            }
            if (empty($name)) {
                $result['errors'][] = "第{$row}行：原片名称不能为空";
                $result['error']++;
                continue;
            }
            if (empty($shortName)) {
...
function importPackages($sheet)
{
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        try {
            ...
            $rackCode = trim($sheet->getCell('G' . $row)->getValue());
            ...
            $rackId = null;
            if (!empty($rackCode)) {
                $rack = fetchRow("SELECT id FROM storage_racks WHERE code = ?", [$rackCode]);
                if (!$rack) {
                    $result['errors'][] = "第{$row}行：库位编码 {$rackCode} 不存在";
                    $result['error']++;
                    continue;
                }
                $rackId = $rack['id'];
            ...
            if ($entryDate instanceof DateTime) {
                ...
                $entryDate = date('Y-m-d', ($entryDate - 25569) * 86400); // Excel日期转换
            } else {
...
<div class="content-section">
    <div class="section-header">
        <p class="section-description">支持导入基地、库位、原片类型、原片包信息等基础数据</p>
    </div>
        ...
        <div class="template-section">
            ...
            <div class="template-info">
                ...
                <ul>
                    ...
                    <li><strong>Sheet4 - 原片包：</strong>包号、原片类型ID、宽度、高度、片数、入库日期、库位编码</li>
                </ul>
...
// 渲染页面
echo renderAdminLayout('数据导入工具', $content, $currentUser, 'tools.php', [], [], $message, $messageType);
?>